...
public class Task11
{
    ...
    {
        System.out.println(findMaxProductOfFourAdjacentNumbers());
    }

    public static int findMaxProductOfFourAdjacentNumbers()
    {
        int grid[][] = new int[][] {
        ...
        {
            for (int j = 0; j < grid.length - runSize; j++)
            {
                int horizontalProduct = 1;
                int verticalProduct = 1;
                int backslashProduct = 1;
                int forwardSlashProduct = 1;
                ...
                for (int k = 0; k < runSize; k++)
                {
                    ...
                    if (i < grid.length - runSize)
                    {
                        backslashProduct *= grid[i + k][j + k];
                        forwardSlashProduct *= grid[i + runSize - k][j + k];
                    }
                }

                maxProduct = Math.max(horizontalProduct, maxProduct);
                maxProduct = Math.max(verticalProduct, maxProduct);
                maxProduct = Math.max(backslashProduct, maxProduct);
                maxProduct = Math.max(forwardSlashProduct, maxProduct);
            }
        }
        return maxProduct;
    }
